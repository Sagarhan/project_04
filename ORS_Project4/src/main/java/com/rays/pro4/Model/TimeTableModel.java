	...
	public void update(TimeTableBean bean) throws ApplicationException, DuplicateRecordException {
		...
		try {
			...
			PreparedStatement pstmt = conn.prepareStatement(
					"update ST_timetable set course_id=?,course_name=?,subject_id=?,subject_name=?,semester=?,"
					+ "exam_date=?,exam_time=?,description=?,created_by=?,modified_by=?,created_datetime=?,modified_datetime=? where ID=?");
